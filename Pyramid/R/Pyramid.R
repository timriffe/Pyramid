...
#'@description    The Pyramid package provides a simple wrapper to \code{barplot()} with several 
#' optional arguments and defaults to quickly plot a population pyramid, and with automatic detection 
#' and plotting of multistate pyramids. The function also gives optional absolute or percent scales, 
#' with flexible age-group widths, optional cohort labels on the right axis, and optional reference lines.
#' @param males either a numeric vector of male population counts by age or a matrix or data.frame of the 
#' male population, where each column is a state (e.g. employed, unemployed).
#' @param females either a numeric vector of female population counts by age or a matrix or data.frame 
#' of the female population, where each column is a state (e.g. employed, unemployed).
#' @param widths numeric vector of the age-interval widths; must be the same length as \code{males} and 
#' \code{females}. If missing, defaults to \code{rep(1,length(males))}. Population counts are always divided 
#' by the interval widths for plotting. This makes bar magnitudes comparable between pyramids with different 
#' age intervals; bars are to be interpreted as single year ages, where all of the single ages within a 5-year 
#' class have the same value. Ages for axes are computed from \code{widths}.
#' @param prop logical. Should the x-axis be in percent or absolute value? Defaults to \code{TRUE}(percent 
#' axis). If absolute, the function tries to guess how many 0s to include in tick labels, and indicates 
#' millions or thousands in the axis labels.
#' @param {fill.males, fill.females} The fill color for the male bars (left side) and female bars (right 
#' side). Can be specified in any way that R accepts. Defaults are \code{"orange"} and \code{"purple"}, 
#' respectively, for simple pyramids, or \code{rainbow(k)} for multistate pyramids, where k is the number 
#' of states (columns in the input data).
#' @param {border.males, border.females} border color for the male and female bars. Can be specified 
#' in any way that R accepts. Default = \code{"transparent"}.   
#' @param grid logical. Defaults to \code{TRUE}. Should reference lines be drawn across the plot for 
#' age, cohort or x axis ticks? 
#' @param grid.lty, grid.col, grid.lwd, grid.bg}
#'        graphical parameters for default reference lines. These get passed on to age, cohort and 
#' vertical reference lines unless specified specifically. \code{grid.lty} is analagous to \code{lty},
#'  \code{grid.col} to \code{col} and so forth. Background color can be specified with 
#' \code{grid.bg}. Defaults are \code{2}, \code{"grey"}, \code{1} and \code{"transparent"}, respectively.
#' @param coh.axis logical. Defaults to \code{FALSE}. Should a cohort axis be drawn on the right side? In 
#' this case, you must also specify \code{year} to correctly compute birth cohorts. Ticks will be 
#' placed automatically on years ending in 0.
#' @param coh.lines logical. Defaults to \code{FALSE}. Should cohort reference lines be drawn across the plot? 
#' If switched to \code{TRUE}, the lines will match tick mark locations (on 0s) and copy the graphical 
#' parameters from \code{grid.}.
#' @param year The data year. This is only necessary if you want a cohort axis. Default value is 
#' 2000, so be careful!
#' @param {coh.lty, coh.col, coh.lwd} graphical parameters for cohort reference lines. Defaults 
#' to the \code{grid.} values.
#' @param coh.lines.min logical. Defaults to \code{FALSE}. Should minor reference lines be drawn
#'  for cohorts as well? 
#' @param coh.at.min optional vector of cohort values at which to draw minor reference lines. 
#' Defaults to cohort years evenly divisible by 5.
#' @param coh.lty.min, coh.col.min, coh.lwd.min graphical parameters for minor cohort reference 
#' lines. Defaults are \code{3}, 
#' \code{coh.col} and $\code{coh.lwd * .8}$, respectively.
#' @param age.lines logical. Defaults to \code{TRUE}. Should age reference lines be drawn 
#' across the plot? Defaults to the value of the age axis ticks and copies the graphical 
#' parameters from \code{grid.}.
#' @param {age.lty, age.col, age.lwd} optional graphical parameters to control the appearance 
#' of age grid reference lines. Defaults come from \code{grid.}
#' @param age.lines.min logical. Defaults to \code{FALSE}. Should minor reference lines be 
#' drawn for ages as well? 
#' @param age.at.min optional vector of age values at which to draw minor reference lines. 
#' Defaults to ages evenly divisible by 5.
#' @param {age.lty.min, age.col.min, age.lwd.min} optional graphical parameters to control 
#' the appearance of minor age reference minor lines. Defaults come from \code{age.}
#' @param v.lines logical. Defaults to \code{TRUE}. Should vertical reference lines be drawn 
#' across the plot? Defaults to the value of the x axis ticks and copies the graphical 
#' parameters from \code{grid.}.
#' @param {v.lty, v.col, v.lwd} optional graphical parameters to control the appearance 
#' of vertical grid reference lines. Defaults come from \code{grid.}
#' @param v.lines.min logical. Defaults to \code{FALSE}. Should minor reference lines be 
#' drawn for vertical grid direction as well? 
#' @param v.at.min optional vector of x axis values at which to draw minor reference lines. 
#' Defaults to $\code{pretty(xlim, n = 25, min.n = 16)}$.
#' @param v.lty.min, v.col.min, v.lwd.min} optional graphical parameters to control the 
#' appearance of minor vertical reference lines. Defaults come from \code{v.}
#' @param main optional plot title, defaults to just telling you the total population. 
#' To not plot a title, simple specify \code{""}.
#' @param cex.main character expansion of plot title. Defaults to 1, normal plot title 
#' size.
#' @param {xlim, ylim} x and y limits for the plot. Remember that even though axis labels 
#' for males on the left side are strictly positive, it is plotted in negative coordinate space
#' @param mar plot margins. Same as \code{par("mar"}. Defaults to \code{c(5,5,5,5)}.
#' @param xlab x axis label. Defaults to the most reasonable of \code{"percent"}, 
#' \code{"population (1000s)"} or \code{"population (millions)"}
#' @param ylab.left left (age) axis label. Defaults to \code{"Age"}
#' @param ylab.right right (cohort) axis label. Defaults to \code{"Cohort"}. Only plots 
#' if you specify $\code{coh.axis = TRUE}$.
#' @param cex.lab character expansion of axis labels. Defaults to 1, normal \code{xlab}
#'  and \code{ylab} sizes. Applies to \code{xlab}, \code{ylab.right} and \code{ylab.left}.
#' @param xax.at optional, values at which to draw x axis ticks. Default makes use of 
#' \code{pretty()}
#' @param xax.lab optional, labels to print on x axis ticks.
#' @param age.at optional, values at which to draw age (left) axis ticks. Defaults to 
#' multiples of 10.
#' @param age.lab optional, labels to print on age axis ticks.
#' @param coh.at optional, values at which to draw cohort (left) axis ticks. Defaults 
#' to multiples of 10
...
#' @param verbose logical. Should informative but potentially annoying messages be 
#' returned when the function does something you might want to know about? Defaults to \code{TRUE}.
